...
import datastructures.ArraysAndStrings

/** Test class for Arrays and Strings solutions in datastructures.ArrayAndString */
class ArraysAndStringsTest extends FunSuite {
  ...
  test("String is detected as having all unique characters") {
    ...
    assert(ArraysAndStrings.allUniqueCharacters(testString))
  }
  ...
  test("CString is reversed") {
    ...
    assert(ArraysAndStrings.reverseCStyleString(toReverse).sameElements(reversed))
  }   
  ...
  test("Duplicate characters in string are removed"){
    ...
    assert(ArraysAndStrings.withoutDuplicates(duplicateString)==uniqueCharString)
  }
  ...
  test("Two strings are anagrams"){
    ...
    assert(ArraysAndStrings.areAnagrams(anagram1, anagram2))
  }
  ...
  test("Two strings are not anagrams"){
    ...
    assert(!ArraysAndStrings.areAnagrams(notAnagram1, notAnagram2))
  }
  ...
  test("All spaces are replaced with %20"){
    ...
    assert(ArraysAndStrings.replaceAllSpacesWithUrlEncoding(withSpaces)==withPercent20)
  }
  ...
  test("Matrix rotated by 90 degrees"){
    ...
    ArraysAndStrings.rotateMatrix90Degrees(matrix)
  ...
  test("Rows and Columns zeroed properly for a zero entry"){
    ...
    ArraysAndStrings.zeroRowColumnOfZeroEntries(matrix)
  ...
  test("String is rotation of another"){
    ...
    assert(ArraysAndStrings.isRotation(string1, string2))
  }
